  ...
  mkDerivation =
    ...
    stdenv.mkDerivation {
      ...
      installPhase = let
        ...
        revision_url = "https://github.com/erictapen/vcc-refa/" + (if revision == "dirty" then "" else "commit/${revision}");
      in ''
        sed -i 's|GIT_REV|${revision}|g' src/Main.elm
        sed -i 's|GITHUB_URL|${revision_url}|g' src/Main.elm
